...
<section id="" class="navigation pb-0">
    <div class="container">
        <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-between gap-3 rounded shadow p-4">
            <h2 class="h4 mb-0"><?= !empty($data['query']) ?  "Hasil dari : " . ucwords($data['query']) : "Daftar Semua Modul" ?></h2>
            <div class="d-flex flex-wrap flex-lg-nowrap align-items-center gap-3">
                <div class="dropdown w-100 order-2 order-lg-1">
                    <button class="btn btn-outline-secondary dropdown-toggle rounded w-100" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                        <i class="bi bi-mortarboard-fill me-2"></i></i></i>Semua
                    </button>
                ...
                <div class="dropdown w-100 order-2 order-lg-1">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="<?= !empty($data['query']) ? base_url('modul?q=' . $data['query'] . '&f=terbaru') : base_url('modul?f=terbaru') ?>">Terbaru</a></li>
                        <li><a class="dropdown-item" href="<?= !empty($data['query']) ? base_url('modul?q=' . $data['query'] . '&f=terlama') : base_url('modul?f=terlama') ?>">Terlama</a></li>
                    </ul>
                </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <?php forEach($data['results'] as $modul): ?>
                            <tr>
                                <td class="align-middle" style="width:30%"><strong><?= $modul['title'] ?></strong></td>
                                <td class="align-middle" style="width:20%"><?= $modul['major'] ?></td>
                                <td class="align-middle" style="width:20%"><a href="<?= base_url('modul?q=' . $modul['teacher']) ?>"><?= $modul['teacher'] ?></a></td>
                                <td class="align-middle" style="width:20%"><a href="<?= base_url('modul?q=' . $modul['writer']) ?>"><?= $modul['writer'] ?></a></td>
